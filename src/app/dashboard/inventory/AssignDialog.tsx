    ...
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/40 sm:items-center">
      <div className="w-full max-w-md rounded-t-3xl bg-white p-5 ring-1 ring-neutral-200 sm:rounded-3xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-bold">Ajouter à une liste</h2>
          <button type="button" onClick={onClose} className="text-neutral-500" aria-label="Fermer">✕</button>
        </div>

        <p className="text-sm text-neutral-600">
          {itemIds.length} objet{itemIds.length > 1 ? "s" : ""} à déplacer
        </p>
        ...
        <div className="mt-4 max-h-[50vh] space-y-2 overflow-y-auto">
          ...
          {lists && lists.length === 0 && (
            <div className="rounded-2xl bg-neutral-50 p-4 text-sm text-neutral-600">
              Aucune liste. Crée-en une d’abord.
            </div>
          )}
        ...
        <div className="mt-4 border-t border-neutral-100 pt-3">
          <button
            type="button"
            onClick={() => router.push(`/dashboard/lists/new?from=inventory&items=${encodeURIComponent(itemIds.join(","))}`)}
